...
def tutorial_1(*, session: Session = Depends(get_session)):
    ...
    team_preventers = Team(name="Tutorial Preventers", headquarters="Sharp Tower")
    team_z_force = Team(name="Tutorial Z-Force", headquarters="Sister Margaret’s Bar")
    session.add(team_preventers)
    session.add(team_z_force)
    session.commit()

    hero_deadpond = Hero(
        name="Tutorial Deadpond", secret_name="Dive Wilson", team_id=team_z_force.id
    )
    hero_rusty_man = Hero(
    ...
    hero_spider_boy = Hero(name="Tutorial Spider-Boy", secret_name="Pedro Parqueador")
